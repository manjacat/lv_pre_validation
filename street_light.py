...
def st_light_device_id_format_message(device_id):
    e_msg = rps_device_id_format_message(layer_name, device_id, st_light_device_id_format_code)
    return e_msg
...
def st_light_cont_dev_message(device_id):
    ...
    e_msg = st_light_cont_dev_code + ',' + str(device_id) + ',' + layer_name + ': ' + str(
        device_id) + ' cont_device should be PANEL if dmd_pnt_id is not null' + ',' + str(longitude) + ',' + str(
        latitude) + ' \n'
    return e_msg
